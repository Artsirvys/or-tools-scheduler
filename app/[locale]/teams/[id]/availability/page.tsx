...
import { ArrowLeft, Users, Check, X, Star, Clock, AlertCircle, UserCheck, UserX, Calendar, Mail } from "lucide-react"
...
type Availability = {
  ...
  shift_id: string
  date: string
  status: 'available' | 'unavailable' | 'priority' | 'vacation' | 'conference'
  created_at: string
}
...
export default function TeamAvailabilityPage({ params }: { params: Promise<{ id: string }> }) {
  const tRemind = useTranslations("hostTeamAvailability")
  ...
  const handleRemindMissingAvailability = async () => {
    if (!teamId || !selectedMonth || remindLoading) return
    setRemindLoading(true)
    try {
      const res = await fetch(`/api/teams/${teamId}/remind-availability`, {
        method: "POST",
      ...
      if (!res.ok) {
        if (data.error === "no_shifts") {
          alert(tRemind("remindNoShifts"))
        } else {
          alert(tRemind("remindFailed"))
        }
        return
      }
      ...
      if (pending === 0) {
        alert(tRemind("remindNone"))
        return
      }

      let message = ""
      if (sent > 0) {
        message = tRemind("remindSuccess", { count: sent })
      }
      if (failed > 0 || skipped > 0) {
        const extra = tRemind("remindPartial", { failed, skipped })
        message = message ? `${message} ${extra}` : extra
      }
      if (!message) {
        message = tRemind("remindNone")
      }
      alert(message)
    } catch {
      alert(tRemind("remindFailed"))
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="flex flex-col sm:flex-row gap-4 mb-8 items-end">
          ...
          <Button
            ...
            disabled={remindLoading || !teamId || !selectedMonth}
            onClick={handleRemindMissingAvailability}
          >
            <Mail className="h-4 w-4 mr-2" />
            {remindLoading ? tRemind("remindSending") : tRemind("remindButton")}
          </Button>
        </div>

...
import { Calendar, Clock, AlertCircle, Bell, LogOut, Mail } from "lucide-react"
...
export default function ParticipantDashboard() {
  ...
  const handleShiftChangeRequest = async (shiftId: string, date: string, teamId: string) => {
    try {
      router.push(`/participant/shift-change?shiftId=${shiftId}&date=${date}&teamId=${teamId}`)
    } catch (error) {
      console.error('Error navigating to shift change:', error)
      alert(tx("alerts.openShiftChangeFailed", "Failed to open shift change request"))
    }
  }
    ...
    try {
      ...
      const notificationsData = requests.map(request => {
        ...
        const requesterName = requester?.last_name 
          ...
          : requester?.first_name || tx("notifications.unknownUser", "Unknown User")

        const originalDate = assignment?.date
        const requestedDate = request.requested_date
        const originalShiftName = originalShift?.name || tx("notifications.unknownShift", "Unknown Shift")
        const requestedShiftName = requestedShift?.name || tx("notifications.unknownShift", "Unknown Shift")

        const message = `${requesterName} ${tx("notifications.wantsToSwap", "wants to swap")} ${originalShiftName} ${tx("notifications.on", "on")} ${new Date(originalDate).toLocaleDateString(dateLocale, { month: "short", day: "numeric" })} ${tx("notifications.withYour", "with your")} ${requestedShiftName} ${tx("notifications.on", "on")} ${new Date(requestedDate).toLocaleDateString(dateLocale, { month: "short", day: "numeric" })}`
  ...
  const handleAcceptRequest = async (notificationId: string) => {
    try {
      ...
      if (!user) {
        alert(tx("alerts.userNotAuthenticated", "User not authenticated"))
        return
      }
      ...
      if (currentError) {
        ...
        alert(tx("alerts.processRequestFailed", "Failed to process request"))
        return
      }
      ...
      alert(tx("alerts.acceptedSuccess", "Shift change accepted! Your schedule has been updated."))
    } catch (error) {
      console.error('Error accepting request:', error)
      alert(tx("alerts.acceptFailed", "Failed to accept request"))
    }
  }
  ...
  const handleDeclineRequest = async (notificationId: string) => {
    try {
      ...
      alert(tx("alerts.declinedSuccess", "Shift change request declined."))
    } catch (error) {
  ...
  const fetchUpcomingShifts = useCallback(async (userId: string) => {
    try {
      ...
      const shiftsData = assignments.map(assignment => {
          ...
          let teamDisplayName = tx("notifications.unknownTeam", "Unknown Team")
          let teamId = ''
          ...
          return {
            ...
            shift: shift?.name || tx("notifications.unknownShift", "Unknown Shift"),
            time: shift ? `${shift.start_time} - ${shift.end_time}` : tx("notifications.unknownTime", "Unknown Time"),
    ...
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 dark:text-gray-300">{tx("loading", "Loading dashboard...")}</p>
        </div>
      </div>
    )
  }
            ...
            <div className="w-full sm:w-auto sm:flex sm:items-center sm:gap-3">
              <div className="grid grid-cols-2 gap-2 sm:flex sm:items-center sm:gap-3">
                ...
                <Button 
                  ...
                  className="w-full justify-center text-sm sm:w-auto font-medium text-gray-700 dark:text-gray-300 hover:text-gray-900 dark:hover:text-gray-100 hover:bg-gray-100 dark:hover:bg-gray-700 flex items-center gap-1.5"
                >
                  {tx("sendFeedback", "Send Feedback")} <Mail className="h-4 w-4" />
                </Button>
              </div>
      ...
      <div className="container mx-auto px-4 py-6 max-w-3xl">
        ...
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white mb-1">
            {tx("hello", "Hello")}, {userName || tx("participant", "Participant")}!
          </h1>
          <p className="text-gray-600 dark:text-gray-300">{tx("overviewSubtitle", "Here is your shift overview")}</p>
        </div>
          ...
          <Link href="/participant/availability">
            <Card className="hover:shadow-lg transition-all cursor-pointer border-2 hover:border-blue-200">
              <CardContent className="p-6 text-center">
                ...
                <h3 className="text-lg font-semibold mb-2">{tx("setAvailability", "Set My Availability")}</h3>
                <p className="text-sm text-gray-600 dark:text-gray-300">{tx("setAvailabilityDesc", "Mark when you can work")}</p>
              </CardContent>
            </Card>
          </Link>
          ...
          <Link href="/participant/shift-change">
            <Card className="hover:shadow-lg transition-all cursor-pointer border-2 hover:border-orange-200">
              <CardContent className="p-6 text-center">
                ...
                <h3 className="text-lg font-semibold mb-2">{tx("requestShiftChange", "Request Shift Change")}</h3>
                <p className="text-sm text-gray-600 dark:text-gray-300">{tx("requestShiftChangeDesc", "Swap or change shifts")}</p>
              </CardContent>
            </Card>
          </Link>
          ...
          <Link href="/schedule/participant/1">
            <Card className="hover:shadow-lg transition-all cursor-pointer border-2 hover:border-purple-200">
              <CardContent className="p-6 text-center">
                ...
                <h3 className="text-lg font-semibold mb-2">{tx("viewSchedule", "View Schedule")}</h3>
                <p className="text-sm text-gray-600 dark:text-gray-300">{tx("viewScheduleDesc", "See your assigned shifts")}</p>
              </CardContent>
            </Card>
          </Link>
        </div>
              ...
              <CardContent>
                {loading ? (
                  <div className="text-center py-8">
                    ...
                    <p className="text-gray-600">{tx("loadingShifts", "Loading your shifts...")}</p>
                  </div>
                ) : userStatus === 'no-team' ? (
                  <div className="text-center py-8">
                    <Clock className="h-12 w-12 text-gray-400 mx-auto mb-4" />
                    <p className="text-gray-600 mb-2">{tx("noTeam", "You are not assigned to a team")}</p>
                    <p className="text-sm text-gray-500">{tx("noTeamDesc", "Contact your administrator to be added to a team")}</p>
                  </div>
                ) : userStatus === 'no-shifts' ? (
                  <div className="text-center py-8">
                    <Calendar className="h-12 w-12 text-gray-400 mx-auto mb-4" />
                    <p className="text-gray-600 mb-2">{tx("noShifts", "No upcoming shifts scheduled")}</p>
                    <p className="text-sm text-gray-500">{tx("noShiftsDesc", "Here you will see your shifts when they are assigned to you")}</p>
                  </div>
                ) : (
                  <div className="space-y-3">
                    {upcomingShifts.map((shift) => {
                      const formattedDate = new Date(shift.date).toLocaleDateString(dateLocale, {
                        month: "short",
                        day: "numeric",
                        weekday: "short"
                      })
                        ...
                        >
                          <div className="flex-1">
                            <div className="flex items-center space-x-2 mb-1">
                              ...
                              >
                                {shift.status === "confirmed" ? tx("status.confirmed", "Confirmed") : tx("status.changeRequested", "Change Requested")}
                              </Badge>
                            </div>
            ...
            <Card>
              <CardHeader>
                <CardTitle className="text-lg flex items-center">
                  ...
                  {tx("newNotifications", "New Notifications")}
                </CardTitle>
              </CardHeader>
              <CardContent>
                {notifications.length === 0 ? (
                  <div className="text-center py-8">
                    <p className="text-gray-600 mb-2">{tx("noNotifications", "No new notifications")}</p>
                    <p className="text-sm text-gray-500">{tx("noNotificationsDesc", "You'll see shift change requests here when they come in")}</p>
                  </div>
                ) : (
                  <div className="space-y-3">
        ...
        {activeTab === "team" && (
          <div className="space-y-6">
            ...
            {userTeams.length > 1 && (
              <Card>
                <CardHeader>
                  <CardTitle className="text-lg">{tx("selectTeamToView", "Select Team to View")}</CardTitle>
                  <CardDescription>{tx("selectTeamToViewDesc", "Choose a team to see its members and details")}</CardDescription>
                </CardHeader>
                <CardContent>

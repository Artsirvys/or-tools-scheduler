...
export default function HostSchedulePage({ params }: SchedulePageProps) {
  ...
  const getAvailableMembers = (shiftId: string, date: string) => {
    const availableMembers = availabilityData
      ...
      .sort((a, b) => {
        // Sort by priority: priority > available > blocking types
                                        ...
                                        <SelectItem key={member.userId} value={member.userId}>
                                          <div className="flex items-center space-x-2">
                                            ...
                                            <span className={`text-xs px-1 py-0.5 rounded ${
                                              ...
                                              member.status === 'unavailable' ? 'bg-red-100 text-red-800' :
                                              member.status === 'vacation' ? 'bg-amber-100 text-amber-800' :
                                              member.status === 'conference' ? 'bg-purple-100 text-purple-800' :

...
export async function GET(request: NextRequest) {
  ...
  if (code) {
    ...
    if (error) {
      ...
      const signInUrl = userEmail 
        ? `/auth/signin?message=${messageType}&email=${encodeURIComponent(userEmail)}`
        : `/auth/signin?message=${messageType}`
    ...
    if (data?.user) {
        ...
        if (!existingProfile) {
          ...
          if (profileError) {
            ...
            if (plan || accountType === 'host') {
              try {
                const trialResponse = await fetch(`${origin}/api/create-trial`, {
                  method: 'POST',
                  headers: {
                  ...
                  body: JSON.stringify({
                    userId: user.id,
                    planId: plan || 'default-host-plan'
                  })
                })
                
                if (!trialResponse.ok) {
                  console.error('Trial creation failed in callback')
                }
              } catch (trialError) {
      ...
      const signInUrl = `/auth/signin?email=${encodeURIComponent(user.email || '')}&message=email_confirmed`
      return NextResponse.redirect(`${origin}${signInUrl}`)
    } else {
      console.error('No user data after code exchange')
      // Redirect to signin with error message instead of error page
      return NextResponse.redirect(`${origin}/auth/signin?message=confirmation_error`)
    }
  }
  ...
  return NextResponse.redirect(`${origin}/auth/signin?message=no_code`)
}
